...
import { buildIndex, formatSearchResults, getBookmarkById, getStats, listBookmarksWithFilters, searchBookmarks, applyRegexClassifications, openClassifyDb, saveClassifyDb, getCategoryCounts, getDomainCounts, } from "./bookmarks-db.js";
...
function printDistribution(items) {
...
async function runLlmClassify(options) {
...
`);
    ...
    program
        ...
        .option("--limit <n>", "Max results", (value) => Number(value), 20)
    ...
    program
        ...
        .option("--after <date>", "Filter to items on or after YYYY-MM-DD")
        .option("--before <date>", "Filter to items on or before YYYY-MM-DD")
        .option("--limit <n>", "Max results", (value) => Number(value), 30)
    ...
    program
        ...
        if (options.regex) {
            const records = await readJsonLines(bookmarksCachePath());
