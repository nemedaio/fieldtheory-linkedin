...
function buildCronLine(options) {
    const bin = resolveFtliBin();
    const useNpx = bin === "npx";
...
export async function enableSchedule(options) {
    return isMac ? enableLaunchd(options) : enableCron(options);
}
async function enableLaunchd(options) {
...
async function enableCron(options) {
