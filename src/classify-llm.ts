...
async function detectEngine(): Promise<Engine> {
  ...
  throw new Error(
    "No LLM CLI found. Install the Claude CLI (claude) or Codex CLI (codex) to use LLM classification.",
  );
}
...
async function promptLlm(engine: Engine, prompt: string): Promise<string> {
...
function mapDbRows(rawRows: unknown[][]): DbRow[] {
  return rawRows.map((r) => ({
    id: String(r[0]),
    text: String(r[1] ?? ""),
    author_slug: r[2] as string | null,
    author_name: r[3] as string | null,
    categories: r[4] as string | null,
  }));
}

function getAllRows(db: Database): DbRow[] {
  return mapDbRows(db.exec(`${ROW_QUERY} ORDER BY rowid`)[0]?.values ?? []);
}

function getUnclassifiedRows(db: Database, column: "primary_category" | "primary_domain"): DbRow[] {
  return mapDbRows(
    db.exec(`${ROW_QUERY} WHERE ${column} IS NULL OR ${column} = 'unclassified' ORDER BY rowid`)[0]?.values ?? [],
  );
}

function buildCategoryPrompt(batch: DbRow[]): string {
...
function buildDomainPrompt(batch: DbRow[]): string {
  const lines = batch.map((row, i) => {
    const author = row.author_slug ?? row.author_name ?? "unknown";
...
export interface ClassifyLlmProgress {
  phase: Phase;
  batch: number;
  totalBatches: number;
  classified: number;
...
export interface ClassifyLlmOptions {
  onProgress?: (progress: ClassifyLlmProgress) => void;
  all?: boolean;
}

interface BatchPhaseConfig {
  phase: Phase;
  buildPrompt: (batch: DbRow[]) => string;
  resultKey: "categories" | "domains";
  columns: { list: string; primary: string };
}
...
async function processBatches(
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const results = parseLlmJsonArray(raw) as Array<Record<string, unknown>>;
      const batchIds = new Set(batch.map((r) => r.id));

      for (const item of results) {
        const id = item.id as string | undefined;
        if (!id || !batchIds.has(id)) continue;
        const values = (item[config.resultKey] as string[] | undefined ?? [])
        ...
        db.run(
          `UPDATE bookmarks SET ${config.columns.list} = ?, ${config.columns.primary} = ? WHERE id = ?`,
          [values.join(","), primary, id],
        );
        classified += 1;
      }
    } catch (err) {
      console.error(`  ${config.phase} batch ${batchNum} failed: ${(err as Error).message}`);
    }
  }
...
export async function classifyWithLlm(
  db: Database,
  options: ClassifyLlmOptions = {},
): Promise<{ categorized: number; domained: number }> {
