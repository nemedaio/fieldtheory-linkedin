import type { BookmarkCategory, LinkedinBookmarkRecord } from "./types.js";
...
const CATEGORY_PATTERNS: Array<{ category: BookmarkCategory; patterns: RegExp[] }> = [
...
function extractHostnames(links: string[]): Set<string> {
...
export function classifyBookmark(record: LinkedinBookmarkRecord): ClassifyResult {
  const text = record.text;
  const hosts = extractHostnames(record.links);
  const scores = new Map<BookmarkCategory, number>();
  ...
  for (const host of hosts) {
    if (TOOL_DOMAINS.has(host)) scores.set("tool", (scores.get("tool") ?? 0) + 2);
    if (RESEARCH_DOMAINS.has(host)) scores.set("research", (scores.get("research") ?? 0) + 2);
    if (COMMERCE_DOMAINS.has(host)) scores.set("commerce", (scores.get("commerce") ?? 0) + 2);
  }
...
export function classifyCorpus(records: LinkedinBookmarkRecord[]): Map<string, ClassifyResult> {
